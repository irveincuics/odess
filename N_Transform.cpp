...
public:
    N_Transform(int num_sketch = 12, int num_group = 3, int sliding_window = 32)
            : num_sketch(num_sketch), num_group(num_group), rabin(sliding_window) {
    ...
    vector<unsigned long long> build_sketch(const vector<uint8_t>& byte_data) {
        ...
        for (auto ch : byte_data) {
            rabin.update(ch);
            unsigned long long fingerprint = rabin.get_hash();
    ...
    unsigned long long grouping(vector<unsigned long long> feature_list) {
        ...
        for (auto feature : feature_list) {
            ...
            for (int i = 0; i < 8; ++i) {
                rabin.update((uint8_t)(feature_bin >> (8 * i)));
            }
        }
